...
Final Deep Search reports ask the writer to emit clickable inline source IDs such as `[S1](https://example.com)` whenever a source URL is available, followed by a `Sources` section with `<div class="source-card" data-source-id="S1">...</div>` blocks. The chat renderer turns those inline links into compact source chips and renders source-card blocks as high-visibility clickable source cards while preserving `data-source-id` for frontend parsing.
