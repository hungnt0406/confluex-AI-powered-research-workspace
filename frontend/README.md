...
- First user message without an active project:
  ...
  2. `POST /projects/{id}/run/stream` — Searcher → Reader pipeline over SSE; the `papers` event populates the right panel as soon as ranking commits, while `summary` events update individual cards progressively.
...
- Writer workspace: `/writer` lists user-owned writer documents without requiring an active project. New documents can start independently, and `/writer/[documentId]` attaches sources through writer-owned PDF upload, source search, manual attach APIs, or optional project-paper import. Section drafts are gated by a generated or user-edited outline that must be approved before drafting; research and survey Methods/Results outlines render as LaTeX subsections. The Monaco editor also exposes targeted editor-agent previews for selected-span fixes, prompted edits, paragraph insertion, and pasted-result incorporation; accepted previews apply through the backend section edit endpoint so version history is preserved.
