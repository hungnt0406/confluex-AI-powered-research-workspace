...
- Admin usage monitor: `/admin/usage` checks `GET /admin/access`, then reads `GET /admin/token-usage` as a global dashboard for allowlisted admins with preset or custom date ranges. `/admin/usage/users` reuses the same endpoint for selected-user analysis with a searchable user picker and a `user_id` query string deep link.
