...
## 2026-05-14T23:20:46+07:00
- **Request:** Iteratively fix the visual-mode prose editor and bring it to parity with Source mode: typing must work, AI Edit overlay must work over the prose surface, Undo/Redo, and the Edit dialog must not get stuck.
- **Files changed:** `frontend/components/WriterProseEditor.tsx`, `frontend/components/WriterEditorOverlay.tsx`, `frontend/components/WriterWorkspace.tsx`, `frontend/lib/dom-latex-map.ts` (new), `frontend/lib/prose-editor-adapter.ts` (new), `tests/test_frontend_writer_static.py`, `JOURNAL.md`, `AI_WORKLOG.md`.
- **Current status:**
  - Replaced `dangerouslySetInnerHTML` in the prose editor with a one-shot `useEffect` that writes innerHTML manually so React never re-applies it during re-renders. Typing now persists. The mount effect exposes the contenteditable node via an `onMount(el)` prop.
  - Added `frontend/lib/dom-latex-map.ts` with `domPositionToLatexOffset` / `latexOffsetToDomPosition` plus `offsetToLineColumn` / `lineColumnToOffset` / `getLineContent` / `getLineCount`. Headings, `\textbf{}` / `\emph{}` wrappers, atomic `\cite{}` / `\todo{}` chips, `<br>`s, and block separators (`\n\n`) all participate in the offset accounting.
  - Added `frontend/lib/prose-editor-adapter.ts` that wraps the contenteditable as `MonacoEditorLike`. The selection listener filters to events whose range is inside the editor so focusing the dialog textarea doesn't clear the saved selection.
  - `WriterEditorOverlay` now resets the panel inside the preview `catch` block (failed previews no longer strand the user) and registers a document `mousedown` listener that resets `panelMode` whenever the user clicks back inside the editor — fixing the "edit dialog follows my selection" bug.
...
## 2026-05-16T02:49:54+07:00
- Request: Verify the app still works after Antigravity's terms-enforcement edits, then fix the "Couldn't reach the server" error blocking signup.
- Files changed: `tests/test_auth.py`, `tests/test_credits.py`, `frontend/components/AuthProvider.tsx`, `JOURNAL.md`.
- Current status: Reviewed Antigravity's four-file change set — adds `agreed_to_terms` to `AuthRequest`/`GoogleAuthRequest`, rejects registration when missing, blocks Google auto-register without agreement, gracefully switches Google sign-in to register mode on 400, disables the Google button until the box is checked. Updated existing tests that POSTed to `/auth/register` without the new field, plus added two new guard tests (`test_register_requires_terms_agreement`, `test_register_rejects_explicit_false_agreement`). Replaced `const json: any` in `AuthProvider` with `Record<string, unknown>`. `tsc --noEmit` and `ruff`/`mypy` on the two changed backend files are clean; `pytest tests/test_auth.py tests/test_credits.py` → 8 passed. Frontend "Couldn't reach the server" error was correct: local Postgres 17 (Homebrew) was not running and the `postgres` role + `literature_review` DB didn't exist. Started Postgres via `pg_ctl` (brew services failed because `$HOME` is on an external volume that `launchctl` rejects), created the role + both DBs, ran `alembic upgrade head`. Live POST to `/auth/register` with `agreed_to_terms=true` returns 201; without it returns 400 with the expected detail.

## 2026-05-16T04:07:06+07:00
